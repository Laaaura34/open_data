...
app.post('/covoit', function(req, res){
    let codeInsee = req.body.CodeInsee;
    ...
    res.send(tableau); // Affichage du tableau au format json
})

app.get('/prix', function(request, response){
    const codeInsee = request.query.codeInsee;

    const file = XLSX.readFile('data/dvf-communes-2019.xlsx')
    ...
    // json1['ID'] = insee['A' + i]["v"]; // On créer un tableau json vide en récupérant comme identifiant de chaque tableau : le code insee. Création d'un identifiant unique.
        json1['codeInsee'] = insee['B' + i]["v"]; // On rempli le tableau json de la ligne i avec l'observation du bureau de vote,
